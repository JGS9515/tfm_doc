...
\subsection{Motivación y diferencias respecto al modelo SNN base}

Como se describió en la \hyperref[sec:arquitectura-snn-base]{Sección 2.6}, el modelo SNN base para detección de anomalías consta de una arquitectura bicapa (A-B) donde la capa de entrada emplea codificación por cuantiles y la capa de procesamiento contiene neuronas LIF con conexiones recurrentes entrenadas mediante STDP modificado. Si bien esta arquitectura demuestra la viabilidad de aplicar SNNs a la detección de anomalías en series temporales, se identificaron varias limitaciones fundamentales que motivan el desarrollo de un modelo mejorado.

La motivación principal para desarrollar este modelo híbrido surge directamente de las limitaciones expuestas en el \hyperref[chap:fundamentos]{Capítulo 2}. Específicamente, la ausencia de mecanismos de optimización automatizada de hiperparámetros y de procesamiento jerárquico puede conducir a un rendimiento subóptimo en escenarios con ruido o alta variabilidad.

El modelo híbrido propuesto aborda estas limitaciones mediante las siguientes innovaciones arquitecturales:

\begin{itemize}
    \item Procesamiento jerárquico: Se integra una capa convolucional (C) después de la capa B, creando una arquitectura híbrida SNN-CNN. La capa C aplica convoluciones sobre los impulsos de B para capturar patrones locales utilizando kernels configurables (Gaussiano, Laplaciano, Mexican Hat, Box).
    
    \item Optimización automatizada con Optuna: Los hiperparámetros (nu1, nu2, umbral, decaimiento, parámetros de kernel) se optimizan automáticamente para maximizar métricas como F1-score, eliminando la necesidad de ajuste manual.
    
    \item Rutas duales de procesamiento: El sistema permite evaluar las salidas de las capas B y C en paralelo, seleccionando automáticamente la representación más informativa según criterios de MSE y métricas de clasificación.
    
    \item Filtrado adaptativo de ruido: Las convoluciones en la capa C actúan como filtros temporales que suavizan fluctuaciones espurias, mejorando la robustez en conjuntos de datos ruidosos.
    
    \item Mejora en generalización: Soporte para múltiples conjuntos de datos (IoPS, CalIt2) con técnicas de expansión de etiquetas y padding para manejar secuencias de longitud variable.
\end{itemize}

En términos de implementación, las diferencias clave incluyen la adición de monitores específicos para la capa C, tres modos de procesamiento convolucional (direct, weighted\_sum, max) que serán explicados en la \hyperref[subsec:modos-procesamiento]{Sección 3.5.5}, y evaluación dual de métricas tanto para la capa B como para la capa C. Esta arquitectura híbrida resulta en un modelo más flexible, manteniendo la eficiencia computacional inherente a las SNNs mientras incorpora las capacidades de extracción de características de las redes convolucionales.
...
Estas tablas (\ref{tab:comparacion-arquitecturas-1} y \ref{tab:comparacion-arquitecturas-2}) resaltan cómo el modelo híbrido extiende la original sin alterar su núcleo, pero modificando donde es necesario para mejorar el rendimiento.
...
\hline\hline
\textbf{Aspecto} & \textbf{Modelo SNN Base (A--B)} & \textbf{Modelo Híbrido (A--B--C)} \\
\hline
\textbf{Tipo de capas} & 
...
\hline\hline
\end{tabularx}
\caption{Comparación entre modelo SNN base y modelo híbrido — Parte 1: arquitectura, conexiones y optimización.}
...
Basado en umbral estadístico en impulsos de B.  
Métricas: Precisión, Recall, F1. & 
Evaluación dual: Impulsos de B y C.  
...
\hline\hline
\end{tabularx}
\caption{Comparación entre modelo SNN base y modelo híbrido — Parte 2: evaluación, implementación y ventajas.}
...
\label{subsec:modos-procesamiento}


% Modalidades de Fusión
% direct: Usa directamente los impulsos de la capa C
...
\begin{itemize}
    \item \textbf{direct}: Usa directamente los impulsos de la capa C
        % \begin{lstlisting}
    ...
    \item \textbf{max}: Elemento máximo entre ambas capas. Preserva la activación más fuerte.
        % \begin{lstlisting}
...
En el primer dominio, la señal de serie temporal es previamente procesada y codificada en impulsos a través de cuantización y agrupamiento temporal. Este bloque prepara los trenes de impulsos que alimentan el núcleo SNN. A continuación, en el segundo dominio, la capa A recibe estos impulsos y los transmite a la capa B, donde se aplica dinámica recurrente LIF con aprendizaje STDP. La salida de B se bifurca: una ruta va directamente a la fase de inferencia sobre impulsos de B, y la otra alimenta el bloque convolucional.

El tercer dominio corresponde a la capa C, que aplica convoluciones 1D sobre los impulsos de la capa B utilizando kernels configurables (Gaussianos, Laplacianos, etc.). Esta capa aporta filtrado adaptativo y extracción de características locales antes de enviar su salida al mecanismo de decisión dual. Finalmente, el sistema evalúa ambas rutas (B y C) según métricas de clasificación y MSE, seleccionando automáticamente la representación más informativa.
...
\subsubsection{Ubicación de la capa convolucional en la arquitectura}

La capa convolucional híbrida se sitúa justo después de la capa B, donde recibe los trenes de impulsos que generan las neuronas LIF recurrentes. De esta forma, puede trabajar con patrones temporales que ya han sido refinados por la dinámica recurrente de la SNN base.

La evidencia experimental respalda esta decisión, las arquitecturas híbridas SNN-CNN obtienen mejores resultados cuando combinan el procesamiento temporal asíncrono de las SNNs con operaciones de convolución espacial \cite{sanaullah_hybrid_2024}. Además, esta configuración preserva la estructura temporal de los impulsos mientras los kernels convolucionales extraen características relevantes.
...
\begin{figure}[!htb]
    ...
    \caption{Arquitectura detallada de la capa convolucional híbrida mostrando el flujo desde los impulsos de la capa B hacia el procesamiento convolucional con kernels adaptativos. Fuente: Elaboración propia.}
    \label{fig:Arquitectura detallada SNN}
\end{figure}

La figura \ref{fig:Arquitectura detallada SNN} ilustra el flujo específico de procesamiento dentro del núcleo SNN, destacando cómo los trenes de impulsos generados por las neuronas LIF de la capa B son procesados por kernels convolucionales especializados antes de generar las salidas de la capa C. Este procesamiento incluye la aplicación de diferentes tipos de kernels (Gaussiano, Laplaciano, etc.) y los modos de fusión (direct, weighted\_sum, max) que permiten la extracción optimizada de características temporales.


\subsubsection {Tipos de kernels implementados}

La selección de kernels convolucionales para el procesamiento de impulsos requirió una aproximación diferenciada de las CNNs tradicionales. Tras la evaluación sistemática de múltiples configuraciones, se determinó que los kernels 1D con tamaños entre 3 y 7 elementos proporcionan el equilibrio óptimo entre capacidad de captura de patrones temporales y eficiencia computacional.

El uso de un kernel gaussiano para la convolución responde a la necesidad de preservar la estructura temporal, a la vez que se suavizan posibles fluctuaciones debidas a la naturaleza discreta de los impulsos individuales. Este tipo de kernel es ampliamente utilizado en procesamiento de señales neuronales precisamente por su capacidad para aproximar la respuesta postsináptica biológica y facilitar el análisis cuantitativo.

Los kernels implementados operan sobre ventanas temporales deslizantes, donde cada kernel \( K_i \) de tamaño \( k \) procesa secuencias de impulsos según:
\begin{equation}
    S_{\text{conv}}(t) = \sum_{j=0}^{k-1} K_i(j) \cdot S_{\text{input}}(t-j)
\end{equation}
donde \( S_{\text{input}}(t) \) representa el tren de impulsos de entrada y \( S_{\text{conv}}(t) \) la salida convolucionada.

% \subsubsection{Ventana temporal adaptativa}

% La implementación de ventanas temporales adaptativas constituye una característica distintiva del modelo. A diferencia de las ventanas fijas tradicionales, el sistema emplea ventanas que se ajustan dinámicamente según la frecuencia de impulsos detectada, permitiendo una mejor captura de patrones temporales irregulares, típicos en datos de anomalías.

% La ventana temporal $W(t)$ se define como:
% \begin{equation}
%     W(t) = W_{base} + \alpha \cdot f_{impulso}(t-\tau)
% \end{equation}
...
%     \item $f_{impulso}$ es la frecuencia local de impulsos,
%     \item $\tau$ representa el retardo temporal.
% \end{itemize}

\subsubsection {Conversión de Impulsos a Tensor y Gradientes}
Mecanismo de Conversión
La conversión de trenes de impulsos discretos a representaciones tensoriales continuas se implementa mediante :
...
Esta operación toma como entrada la suma temporal de los impulsos generados por la capa B de la red y aplica dicho kernel gaussiano. El resultado es una señal continua, suavizada, que preserva tanto la información temporal como la amplitud relativa de impulsos. La elección del padding 'same' asegura que la longitud de la señal de salida coincida con la original, facilitando comparaciones directas.
...
% 	•	Función de salida: soft-rate vs. frecuencia de impulsos.
...
\label{subsec:complejidad-computacional}
...
El número de operaciones MAC ejecutadas por un modelo es un parámetro clave para la estimación de su eficiencia energética.

\subsubsection{Estimación de MACs para SNNs}

Las SNNs realizan operaciones MAC cuando los voltajes neuronales se actualizan o cuando se emite un impulso. Por tanto, en cada paso temporal se ejecutan:

\begin{itemize}
    \item Operaciones MAC proporcionales al número total de neuronas: $N_A + N_B + N_C$, siendo:
    \begin{itemize}
        \item $N_A$: número de neuronas en la capa de entrada.
        \item $N_B$: neuronas en la capa LIF (100, 200, 400).
        \item $N_C$: neuronas en la capa C (igual a $N_B$).
    \end{itemize}
    
    \item Operaciones inducidas por impulsos:
    \begin{itemize}
        \item $N_A$ operaciones por impulso en la entrada A.
        \item $N_B$ operaciones por cada impulso en la capa B (conexiones recurrentes).
...
donde $s$ es el número de impulsos generados en la capa B por instante (estimado entre 5--10\% de $N_B$). Si $T$ es la longitud temporal (e.g., $T = 250$), entonces:
