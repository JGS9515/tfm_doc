...
\begin{itemize}
    \item \textbf{Modelo SNN base (A--B)}: implementación base del trabajo previo.
    \item \textbf{SNN híbrida (A--B--C)}: modelo propuesto con capa convolucional y búsqueda de hiperparámetros.
    \item \textbf{Baselines TSFEDL}: OhShuLih \cite{OHSHU2022}, KhanZulfiqar \cite{KHAN2022}, ZhengZhenyu \cite{ZHENG2022}, WeiXiaoyan \cite{WEI2022}.
\end{itemize}

\subsection{Datasets y particionado}
Se emplean datasets públicos con etiquetas binarias (0: normal, 1: anomalía). Para cada uno:
\begin{itemize}
...
\begin{itemize}
    ...
    \item Cálculo de cuantiles sobre train únicamente: rango extendido con \texttt{a = 0{,}1} y resolución \texttt{r = 0{,}05}; determina \texttt{snn\_input\_layer\_neurons\_size}.
...
En este TFM se han utilizado dos entornos de trabajo principales: El Proyecto SNN (con dos variantes internas: modelo SNN base (A–B) y SNN híbrida (A–B–C)) y Baselines TSFEDL.

Ambos entornos no se unificaron deliberadamente debido a incompatibilidades de versiones, particularmente relacionadas con PyTorch, NumPy y Pandas. Intentar forzar una convergencia resultaba inviable, dado que no era posible encontrar una combinación de versiones que permitiera ejecutar simultáneamente ambos proyectos. A continuación se muestran las restricciones técnicas más relevantes:
...
\begin{enumerate}
    \item \textbf{Cobertura de capacidad con pocos puntos:} Se obtiene un rango total de $4\times$ en número de neuronas con solo tres configuraciones, reduciendo el número de escenarios a optimizar bajo las restricciones temporales del TFM (R1) y de coste computacional (R4).
    \item \textbf{Análisis de escalado:} La complejidad operacional estimada (ver ecuación de MACs en la \hyperref[subsec:complejidad-computacional]{Sección 3.5.8}) contiene un término dominante cuadrático en $N_B$ (por $N_B^2$ y el producto $N_A N_B$), permitiendo observar cómo cambia el régimen de coste con incrementos estructurados.
...
\paragraph{Justificación de 100 pruebas por configuración:}
\begin{enumerate}
    ...
    \item \textbf{Eficiencia con poda:} La integración de ASHA reduce el número de evaluaciones completas necesarias, redirigiendo recursos a configuraciones más prometedoras \cite{li_system_2020, broutonlab_efficient_2021}.
...
La SNN híbrida (A--B--C) se optimizó con Optuna usando TPE maximizando el F1-score:
\begin{itemize}
