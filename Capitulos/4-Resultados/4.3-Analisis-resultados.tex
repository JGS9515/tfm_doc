...
En esta sección se presentan y discuten los resultados, comparando el modelo SNN base (A--B), la SNN híbrida (A--B--C) y modelos TSFEDL \cite{AGUILERAMARTOS2023223}. Se incluye análisis por dataset, ablasiones y eficiencia. Los tiempos de entrenamiento reportados deben interpretarse considerando que todos los modelos se ejecutaron en CPU (véase la Sección~\ref{subsec:entorno}).
...
\hline\hline
\end{tabular}
\caption{Mejores resultados de detección de anomalías en el dataset IOPS. Se muestran las métricas de calidad para cada modelo evaluado.}
...
\begin{itemize}
    ...
    \item \textbf{Errores}: Recall muy alto (99.4-100\%) pero precisión moderada (26.4\%), lo que resulta en una estrategia conservadora que prioriza detectar todas las anomalías aunque genere falsos positivos. Esta aproximación es apropiada para aplicaciones donde perder una anomalía es muy costoso.
\end{itemize}
...
\hline\hline
\end{tabular}
\caption{Mejores resultados de detección de anomalías en el dataset CalIt2. Se muestran las métricas de calidad para cada modelo evaluado.}
\label{tab:resultados-iops}
\end{table}

\subsection{Comparación con modelos TSFEDL}
Se comparan los siguientes modelos de la librería TSFEDL \cite{AGUILERAMARTOS2023223} basándose en los resultados de los experimentos de evaluación:
\begin{itemize}
